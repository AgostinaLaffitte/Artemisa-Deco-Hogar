...
export class OrdersService {
  ...
  const { 
    ...
    deliveryMethod, address, city, postalCode, notes, paymentMethod 
  } = createOrderDto;
  ...
  try {
    ...
    const ngrokUrl = this.configService.get<string>('NGROK_URL') || 'http://localhost:3005';
    const cleanNgrok = ngrokUrl.trim().replace(/\/+$/, '');

    const preference = new Preference(this.mpClient);

    // 2. Creación con payload directo
    const response = await preference.create({
      body: {
        ...
        notification_url: `${cleanNgrok}/orders/webhook`,
...
async handleWebhook(query: any, body: any) {
  try {
    ...
    return { received: true }; // Se devuelve 200 siempre para que MP no reintente indefinidamente
  }
}
  async findAll() {
